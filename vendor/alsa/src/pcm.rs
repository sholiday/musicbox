...
///
/// On 32-bit ARM the ALSA C API expects `snd_pcm_status_t` to be 8-byte aligned. Without this
/// attribute we would hand libasound an unaligned buffer, which can SIGSEGV when it reads the
...
impl Status {
    ...
    pub fn get_htstamp(&self) -> timespec {
        ...
        {
            // Some 32-bit ARM ALSA builds crash when invoking snd_pcm_status_get_htstamp. Returning
            // a zeroed timestamp keeps callers like CPAL/Rodio functional without triggering the
            // kernel bug.
